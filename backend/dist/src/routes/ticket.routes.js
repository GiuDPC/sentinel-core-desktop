...
router.get('/my-tickets', roleGuard('REQUESTER'), ticketController.findMyTickets);
// GET /api/tickets/assigned — Tickets asignados al técnico
router.get('/assigned', roleGuard('TECHNICIAN'), ticketController.findAssigned);
...
router.post('/:id/reassign', roleGuard('ADMIN'), validate(assignTechnicianSchema), ticketController.reassignTechnician);
// POST /api/tickets/:id/resolve — Técnico envía formulario de cierre
router.post('/:id/resolve', roleGuard('TECHNICIAN'), validate(resolveTicketSchema), ticketController.resolveWithNote);
// POST /api/tickets/:id/confirm — Solicitante confirma o reabre
router.post('/:id/confirm', roleGuard('REQUESTER'), validate(confirmTicketSchema), ticketController.confirmTicket);
