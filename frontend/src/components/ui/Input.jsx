...
export function Input({ type = 'text', name, placeholder, value, onChange, error, label, className = '' }) {
  ...
  return (
    <div className={`flex flex-col gap-1.5 ${className}`}>
      {label && (
        <label 
          htmlFor={name}
          className="font-body text-sm font-medium text-[#011e58] tracking-tight"
        >
          {label}
        </label>
      )}
      <div className="relative">
        ...
        {isPassword && (
          <button
            ...
            className="absolute right-3 top-1/2 -translate-y-1/2 text-gray-400 hover:text-gray-600 focus:outline-none focus:ring-2 focus:ring-[#003091]/20 rounded"
          >
            {showPassword ? (
